...
class sr2:
	...
	def __init__(self, deck, fresh_stack_capacity = 5, number_of_stacks = 4):
		self.name = deck["name"]
	...
	def get_next_item(self):
		...
		self.current_item = self.stacks[current_stack].pop(idx)
		self.current_stack = current_stack
		return [self.qparser(self.current_item[0],self.current_item[1]),
                        self.aparser(self.current_item[0],self.current_item[1])]
